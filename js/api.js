            ...
            competitors: [
              {
                ...
                team: {
                  ...
                  logo: `https://ui-avatars.com/api/?name=${encodeURIComponent(
                    homeTeamName
                  )}&background=15803d&color=fff&bold=true&rounded=true`
                }
              },
              {
                homeAway: 'away',
                ...
                team: {
                  ...
                  logo: `https://ui-avatars.com/api/?name=${encodeURIComponent(
                    awayTeamName
                  )}&background=0369a1&color=fff&bold=true&rounded=true`
                }
              }
            ]
          }
        ...
        typeof LEAGUES !== 'undefined'
          ? LEAGUES.find(
              l =>
                (extractedSlug &&
                  (l.id === extractedSlug ||
                    extractedSlug.startsWith(l.id + '.'))) ||
                (slug !== 'all' && l.id === slug) ||
                (rawName &&
                  l.name.toLowerCase() === rawName.toLowerCase()) ||
                (rawName &&
                  rawName.toLowerCase().includes(l.name.toLowerCase()))
            )
          : null;

      const finalLeagueName =
        foundLeague?.name || rawName || 'Pertandingan';

      const finalLeagueId = foundLeague?.id || extractedSlug || slug;

      const finalLeagueFlag =
        foundLeague?.flag ||
        (typeof getLeagueFlag === 'function'
          ? getLeagueFlag(finalLeagueId)
          : '⚽');

      const finalLeagueLogo =
        foundLeague?.logo ||
        evt.league?.logos?.[0]?.href ||
        rootLeague?.logos?.[0]?.href ||
        '';
...
async function fetchBatchLeagues(leaguesList, getDateStrFn) {
  ...
  if (leaguesList.length >= 10) {
    const promises = dateList.map(d =>
      fetchMatchesByLeagueOrAll('all', d)
    );
  ...
  for (let i = 0; i < leaguesList.length; i += BATCH_SIZE) {
    ...
    const promises = batch.map(async league => {
      ...
      const datePromises = leagueDateList.map(d =>
        fetchMatchesByLeagueOrAll(league.id, d)
      );
...
async function loadMultiTierLeagueLogo(
  img,
  leagueId,
  leagueName,
  primaryUrl
...
function showPlayerCircleFallback(img, pName) {
  ...
  const avatarUrl = `https://ui-avatars.com/api/?name=${encodeURIComponent(
    pName
  )}&background=22c55e&color=ffffff&bold=true&rounded=true&size=128`;
...
async function fetchMatchSummary(leagueId, eventId) {
  if (!leagueId || !eventId) {
    console.error('League ID atau Event ID tidak valid:', {
      leagueId,
      eventId
    });
...
async function fetchAllMatches() {
  ...
  try {
    const targetDate =
      selectedDateFilter || getFormattedDate(new Date());
    ...
    // Jika filter liga asing -> Murni 100% dari ESPN
    else {
      const targets = LEAGUES.filter(
        l => l.id === selectedLeague
      );

      allEvents = await fetchBatchLeagues(
        targets,
        () => targetDate
      );
    }
...
async function fetchLiveMatchesStructured() {
  ...
  try {
    ...
    const dateRangeStr = `${getFormattedDate(
      yesterday
    ...
    const next12h = new Date(
      now.getTime() + 12 * 60 * 60 * 1000
    );

    const finishedEvents = sortEventsByFavoriteAndDate(
      allEvents.filter(e => {
    ...
    const liveEvents = sortEventsByFavoriteAndDate(
      allEvents.filter(
        e => e.status?.type?.state === 'in'
      )
    );

    const upcomingEvents = sortEventsByFavoriteAndDate(
      allEvents.filter(e => {
        const d = new Date(e.date);
    ...
    finishedSec.innerHTML = `
      ...
      <div id="live-finished-grid" class="space-y-2.5 ${
        showFinishedInLive ? '' : 'hidden'
      }"></div>
    `;
    ...
    liveSec.innerHTML = `
      <div class="flex items-center justify-between pb-1 border-b border-white/10 text-slate-300">
        ...
        <span class="text-[10px] bg-red-500/20 text-red-400 border border-red-500/30 px-2 py-0.5 rounded-full font-bold">
          ${liveEvents.length}
        </span>
      </div>
    ...
    upcomingSec.innerHTML = `
      <button onclick="toggleUpcomingInLiveView()" class="w-full bg-[#180d30] border border-white/10 p-3 rounded-xl flex items-center justify-between text-xs text-blue-400 font-bold hover:bg-[#231344] transition shadow-lg">
        <span class="flex items-center gap-2">
          ...
          Pertandingan Mendatang (12 Jam Ke Depan) (${upcomingEvents.length})
        </span>

        <i id="upcoming-toggle-icon" class="fa-solid fa-chevron-${
          showUpcomingInLive ? 'up' : 'down'
        } text-[10px]"></i>
      </button>

      <div id="live-upcoming-grid" class="space-y-2.5 ${
        showUpcomingInLive ? '' : 'hidden'
      }"></div>
    `;
...
async function fetchFavoritedMatchesStructured() {
    ...
    container.innerHTML = `
      <div class="text-center py-12 px-4 text-slate-400 bg-[#180d30] border border-white/10 rounded-2xl">
        ...
        <p class="text-[11px] text-slate-400">
          ...
          pada pertandingan atau klub untuk menampilkannya di sini.
        </p>
      </div>
    `;
  ...
  try {
    ...
    if (finishedEvents.length > 0) {
      ...
      finishedSec.innerHTML = `
        <button onclick="toggleFinishedInFavView()" class="w-full bg-[#180d30] border border-emerald-500/80 p-3 rounded-xl flex items-center justify-between text-xs text-emerald-400 font-bold hover:bg-[#231344] transition shadow-lg">
          <span class="flex items-center gap-2">
            <i class="fa-solid fa-circle-check text-emerald-400"></i>
            Pertandingan Selesai (${finishedEvents.length})
          </span>
    ...
    if (upcomingEvents.length > 0) {
      ...
      const upcomingSec = document.createElement('div');
      upcomingSec.className =
        'space-y-2.5 pt-2 border-t border-white/10';
      ...
      upcomingSec.innerHTML = `
        <button onclick="toggleUpcomingInFavView()" class="w-full bg-[#180d30] border border-white/10 p-3 rounded-xl flex items-center justify-between text-xs text-blue-400 font-bold hover:bg-[#231344] transition shadow-lg">
          <span class="flex items-center gap-2">
            <i class="fa-regular fa-calendar-days text-blue-400"></i>
            Pertandingan Mendatang (${upcomingEvents.length})
          </span>
    ...
    if (finished.length < 5) {
      try {
        const prevRes = await fetch(
          `https://site.api.espn.com/apis/site/v2/sports/soccer/all/teams/${teamId}/schedule?season=${
            currentYear - 1
          }`
        );
        ...
        if (prevRes.ok) {
          ...
          [...finished, ...prevFinished].forEach(e =>
...
async function fetchFormAndH2H(
  leagueId,
  homeTeamId,
  awayTeamId,
  homeName,
              ...
              .map(m => {
                ...
                return `
                  <div class="bg-[#180d30] p-2.5 rounded-xl border border-white/10 flex items-center justify-between text-xs">
                    <span class="text-[9px] text-slate-400 w-1/3">
                      ${matchDate}
                    </span>

                    <div class="flex items-center justify-center gap-1.5 w-2/3">
                      <span class="font-semibold text-slate-200 text-right truncate w-5/12">
                        ${hTeam?.team?.shortDisplayName || ''}
                      </span>

                      <span class="font-bold bg-white/10 px-1.5 py-0.5 rounded text-emerald-400 text-[11px]">
                        ${hTeam?.score || '0'} - ${aTeam?.score || '0'}
                      </span>

                      <span class="font-semibold text-slate-200 text-left truncate w-5/12">
                        ${aTeam?.team?.shortDisplayName || ''}
                      </span>
                    </div>
                  </div>
                `;
